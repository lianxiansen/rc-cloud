package com.rc.cloud.ops.gateway.filter;
...
import com.rc.cloud.common.core.exception.ValidateCodeException;
import com.rc.cloud.common.core.web.CodeResult;
import com.rc.cloud.common.core.web.util.WebFrameworkUtils;
import com.rc.cloud.ops.gateway.config.GatewayConfigProperties;
...
public class ValidateCodeGatewayFilter extends AbstractGatewayFilterFactory<Object> {
	...
	public GatewayFilter apply(Object config) {
		return (exchange, chain) -> {
			...
			boolean isIgnoreClient = configProperties.getIgnoreClients().contains(WebFrameworkUtils.getClientId(request));
			try {
				...
				return response.writeWith(Mono.create(monoSink -> {
					try {
						byte[] bytes = objectMapper.writeValueAsBytes(CodeResult.fail(errMsg));
