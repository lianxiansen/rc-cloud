...
import com.rc.cloud.app.system.biz.mapper.oauth2.OAuth2CodeMapper;
...
class OAuth2CodeServiceImplTest extends BaseDbUnitTest {
    ...
    public void testConsumeAuthorizationCode_expired() {
        ...
        // mock 数据
        OAuth2CodeDO oAuth2CodeDO = randomPojo(OAuth2CodeDO.class);
        oAuth2CodeDO.setCode(code);
        oAuth2CodeDO.setExpiresTime(LocalDateTime.now().minusDays(1));
    ...
    public void testConsumeAuthorizationCode_success() {
        ...
        OAuth2CodeDO oAuth2CodeDO = randomPojo(OAuth2CodeDO.class);
        oAuth2CodeDO.setCode(code);
        oAuth2CodeDO.setExpiresTime(LocalDateTime.now().plusDays(1));
        oauth2CodeMapper.insert(oAuth2CodeDO);

        // 调用
        OAuth2CodeDO result = oauth2CodeService.consumeAuthorizationCode(code);
        assertPojoEquals(oAuth2CodeDO, result);
