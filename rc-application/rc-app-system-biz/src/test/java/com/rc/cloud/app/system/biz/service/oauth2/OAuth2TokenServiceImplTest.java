package com.rc.cloud.app.system.biz.service.oauth2;

import cn.hutool.core.date.LocalDateTimeUtil;
import cn.hutool.core.util.RandomUtil;
import com.rc.cloud.app.system.biz.common.redis.OAuth2AccessTokenRedisDAO;
import com.rc.cloud.app.system.biz.mapper.oauth2.OAuth2AccessTokenMapper;
import com.rc.cloud.app.system.biz.mapper.oauth2.OAuth2RefreshTokenMapper;
...
public class OAuth2TokenServiceImplTest extends BaseDbAndRedisUnitTest {
    ...
    public void testCreateAccessToken() {
        ...
        oAuth2ClientDO.setAccessTokenValiditySeconds(30);
        oAuth2ClientDO.setRefreshTokenValiditySeconds(60);
    ...
    public void testRefreshAccessToken_success() {
        ...
        oAuth2AccessTokenDO.setRefreshToken(refreshToken);
        ...
        // 断言，老的访问令牌被删除
        assertNull(oauth2AccessTokenMapper.selectByAccessToken(oAuth2AccessTokenDO.getAccessToken()));
        assertNull(oauth2AccessTokenRedisDAO.get(oAuth2AccessTokenDO.getAccessToken()));
        // 断言，新的访问令牌
    ...
    public void testGetAccessToken() {
        // mock 数据（访问令牌）
        OAuth2AccessTokenDO oAuth2AccessTokenDO = randomPojo(OAuth2AccessTokenDO.class);
        oAuth2AccessTokenDO.setExpiresTime(LocalDateTime.now().plusDays(1));
        oauth2AccessTokenMapper.insert(oAuth2AccessTokenDO);
        // 准备参数
        String accessToken = oAuth2AccessTokenDO.getAccessToken();

        // 调用
        OAuth2AccessTokenDO result = oauth2TokenService.getAccessToken(accessToken);
        // 断言
        assertPojoEquals(oAuth2AccessTokenDO, result, "createTime", "updateTime", "deleted",
                "creator", "updater");
        assertPojoEquals(oAuth2AccessTokenDO, oauth2AccessTokenRedisDAO.get(accessToken), "createTime", "updateTime", "deleted",
                "creator", "updater");
    }
    ...
    public void testCheckAccessToken_expired() {
        // mock 数据（访问令牌）
        OAuth2AccessTokenDO oAuth2AccessTokenDO = randomPojo(OAuth2AccessTokenDO.class);
        oAuth2AccessTokenDO.setExpiresTime(LocalDateTime.now().minusDays(1));
        oauth2AccessTokenMapper.insert(oAuth2AccessTokenDO);
        // 准备参数
        String accessToken = oAuth2AccessTokenDO.getAccessToken();
    ...
    public void testRemoveAccessToken_success() {
        ...
        // mock 数据（刷新令牌）
        OAuth2RefreshTokenDO refreshTokenDO = randomPojo(OAuth2RefreshTokenDO.class)
                .setRefreshToken(oAuth2AccessTokenDO.getRefreshToken());
        oauth2RefreshTokenMapper.insert(refreshTokenDO);
        // 调用
        OAuth2AccessTokenDO result = oauth2TokenService.removeAccessToken(oAuth2AccessTokenDO.getAccessToken());
        assertPojoEquals(oAuth2AccessTokenDO, result, "createTime", "updateTime", "deleted",
                "creator", "updater");
        // 断言数据
        assertNull(oauth2AccessTokenMapper.selectByAccessToken(oAuth2AccessTokenDO.getAccessToken()));
        assertNull(oauth2RefreshTokenMapper.selectByRefreshToken(oAuth2AccessTokenDO.getRefreshToken()));
        assertNull(oauth2AccessTokenRedisDAO.get(oAuth2AccessTokenDO.getAccessToken()));
    }
