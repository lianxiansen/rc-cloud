...
public class PostServiceImplTest extends BaseDbUnitTest {
    ...
    public void testUpdatePost_success() {
        ...
        // 校验是否更新正确
        SysPostPO post = postMapper.selectById(reqVO.getId());
        assertPojoEquals(reqVO, post);
    }
    ...
    public void testValidatePost_codeDuplicateForUpdate() {
        ...
        SysPostPO codePostDO = randomPostDO();
    ...
    public void testGetPostPage() {
        // mock 数据
        SysPostPO postDO = randomPojo(SysPostPO.class, o -> {
            o.setName("码仔");
        ...
        // 调用
        PageResult<SysPostPO> pageResult = postService.getPostPage(reqVO);
        // 断言
    ...
    public void testGetPostList_export() {
        ...
        // 调用
        List<SysPostPO> list = postService.getPostList(reqVO);
        // 断言
    ...
    public void testGetPostList() {
        // mock 数据
        SysPostPO postDO01 = randomPojo(SysPostPO.class, o -> o.setStatus(CommonStatusEnum.ENABLE.getStatus()));
        postMapper.insert(postDO01);
        // 测试 status 不匹配
        SysPostPO postDO02 = randomPojo(SysPostPO.class, o -> o.setStatus(CommonStatusEnum.DISABLE.getStatus()));
        postMapper.insert(postDO02);
        // 准备参数
        List<String> ids = Arrays.asList(postDO01.getId(), postDO02.getId());

        // 调用
        List<SysPostPO> list = postService.getPostList(ids, singletonList(CommonStatusEnum.ENABLE.getStatus()));
        // 断言
        assertEquals(1, list.size());
    ...
    public void testGetPost() {
        // mock 数据
        SysPostPO dbPostDO = randomPostDO();
        postMapper.insert(dbPostDO);
        // 准备参数
        String id = dbPostDO.getId();
        // 调用
        SysPostPO post = postService.getPost(id);
        // 断言
        assertNotNull(post);
    ...
    public void testValidatePostList_success() {
        // mock 数据
        SysPostPO radomPostDO = randomPostDO();
        radomPostDO.setStatus(CommonStatusEnum.ENABLE.getStatus());
        SysPostPO postDO = radomPostDO;
        postMapper.insert(postDO);
        // 准备参数
    ...
    public void testValidatePostList_notEnable() {
        // mock 数据
        SysPostPO randomPostDO = randomPostDO();
    ...
    @SafeVarargs
    private static SysPostPO randomPostDO(Consumer<SysPostPO>... consumers) {
        Consumer<SysPostPO> consumer = (o) -> {
            o.setStatus(randomCommonStatus()); // 保证 status 的范围
        };
        return randomPojo(SysPostPO.class, ArrayUtils.append(consumer, consumers));
    }
}
