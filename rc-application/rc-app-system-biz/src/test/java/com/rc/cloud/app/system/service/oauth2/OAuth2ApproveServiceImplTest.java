...
import com.rc.cloud.app.system.mapper.oauth2.OAuth2ApproveMapper;
...
import static com.rc.cloud.common.test.core.util.RandomUtils.randomLongId;
...
public class OAuth2ApproveServiceImplTest extends BaseDbUnitTest {
    ...
    public void checkForPreApproval_approve() {
        ...
        when(oauth2ClientService.validOAuthClientFromCache(eq(clientId)))
                .thenReturn(oAuth2ClientDO);
        // mock 数据
        OAuth2ApproveDO oAuth2ApproveDO = randomPojo(OAuth2ApproveDO.class);
        oAuth2ApproveDO.setUserId(userId);
        oAuth2ApproveDO.setUserType(userType);
        oAuth2ApproveDO.setClientId(clientId);
        oAuth2ApproveDO.setScope("read");
        oAuth2ApproveDO.setExpiresTime(LocalDateTimeUtil.offset(LocalDateTime.now(), 1L, ChronoUnit.DAYS));
        oAuth2ApproveDO.setApproved(true); // 同意
    ...
    public void checkForPreApproval_reject() {
        ...
        oAuth2ApproveDO.setScope("read");
        oAuth2ApproveDO.setExpiresTime(LocalDateTimeUtil.offset(LocalDateTime.now(), 1L, ChronoUnit.DAYS));
        oAuth2ApproveDO.setApproved(false); // 拒绝
        oauth2ApproveMapper.insert(oAuth2ApproveDO);
    ...
    public void testGetApproveList() {
        ...
        oAuth2ApproveDO.setClientId(clientId);
        oAuth2ApproveDO.setExpiresTime(LocalDateTimeUtil.offset(LocalDateTime.now(), 1L, ChronoUnit.DAYS));
        oauth2ApproveMapper.insert(oAuth2ApproveDO); // 未过期
        OAuth2ApproveDO clone = ObjectUtil.clone(oAuth2ApproveDO);
        clone.setId(null);
        clone.setExpiresTime(LocalDateTimeUtil.offset(LocalDateTime.now(), -1L, ChronoUnit.DAYS));
        oauth2ApproveMapper.insert(clone); // 已过期

        // 调用
        List<OAuth2ApproveDO> result = oauth2ApproveService.getApproveList(userId, userType, clientId);
        // 断言
        assertEquals(1, result.size());
        assertPojoEquals(oAuth2ApproveDO, result.get(0));
    }
