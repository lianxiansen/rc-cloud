...
public class OAuth2GrantServiceImplTest extends BaseMockitoUnitTest {
    ...
    public void testRevokeToken_success() {
        ...
        oAuth2AccessTokenDO.setClientId(clientId);
        when(oauth2TokenService.getAccessToken(eq(accessToken))).thenReturn(oAuth2AccessTokenDO);
        // mock 方法（移除）
        when(oauth2TokenService.removeAccessToken(eq(accessToken))).thenReturn(oAuth2AccessTokenDO);
