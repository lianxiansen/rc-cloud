...
(
    ...
    `icon`                        varchar(128) DEFAULT NULL COMMENT '图标图片',
    `product_category_page_image` varchar(128) DEFAULT NULL COMMENT '商品分类页面图片URL',
    `product_list_page_image`     varchar(128) DEFAULT NULL COMMENT '商品列表页面图片URL',
    `parent_id`                   varchar(32)  DEFAULT NULL COMMENT '父级id',
    `layer`                       int(11) DEFAULT 0 COMMENT '层级',
    `enabled_flag`                bit      DEFAULT '0' COMMENT '状态 1-正常状态，0-未启用',
    `sort_id`                     int(11) DEFAULT 99 COMMENT '排序',
...
(
    ...
    `lowest_buy`                 int(11) DEFAULT 1 COMMENT '最低起购量',
